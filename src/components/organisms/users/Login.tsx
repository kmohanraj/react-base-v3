...
const Login: FC = () => {
  ...
  const handleOnSubmit = async () => {
    ...
    if (response?.status === STATUS_CODE.STATUS_200) {
      ...
      sessionStorage.setItem(SESSION_STORAGE.IS_FIRST_LOGIN, response?.data?.info?.isFirstLogin)
      ...
      response.data?.info.isFirstLogin ? null : (window.location.pathname = '/');
  ...
  const handleOnResetPassword = async () => {
    const response = await userService.resetPasswordFirstLogin(login, Number(sessionStorage.getItem(CONSTANTS.SESSION_STORAGE.USER_ID_KEY)))
    if (response?.status === STATUS_CODE.STATUS_200) {
      sessionStorage.setItem(SESSION_STORAGE.IS_FIRST_LOGIN, response?.data?.info?.isFirstLogin)
      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
      response?.data?.info.isFirstLogin ? null : window.location.pathname = '/'
        ...
        {isFirst && (
          <div>
            <Input
              ...
              required
              message='Eg: Password@123'
              sufFixIcon={isNewPasswordShow ? Icon.showPassword : Icon.hidePassword}
              suffixOnClick={() => setIsNewPasswordShow(!isNewPasswordShow)}
            />
          </div>
        )}
        <div className="form-submit">
          <Button type="primary" label="Login" onClick={ isFirst ? handleOnResetPassword : handleOnSubmit} />
        </div>
      </div>
    </div>
