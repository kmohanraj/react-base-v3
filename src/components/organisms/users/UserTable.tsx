...
const UserTable = () => {
  const { usersData, isUserActive, isDeleteUser } = useSelector(
    (state: RootState) => state.user
  );
  const currentUserID = sessionStorage.getItem(SESSION_STORAGE.USER_ID_KEY);
  const [isUsersLoading, handleRefreshUserTable] = useItToGetAllUsers(
  ...
  const handleOnRemove = (data: any) => {
    if (data.id !== Number(currentUserID)) {
      setUserId(data?.id);
  ...
  const handleOnStatus = async () => {
    const response = await userService.status(
    ...
    <>
      <TopPanel panelType='top-panel'>
        <div className='top-panel-entity'>
          {useItToPanelTotal(Number(usersData?.length), 'User')}
        </div>
